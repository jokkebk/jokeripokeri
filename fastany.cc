...
int enum_hand_pair(int *h, int s) {
    ...
    if(n==3 && numsel[2] < numsel[3]) std::swap(numsel[2], numsel[3]);
    if(n>1 && numsel[1] < numsel[2]) std::swap(numsel[1], numsel[2]);
    if(numsel[0] < numsel[1]) std::swap(numsel[0], numsel[1]);
    // rinse and repeat to get 122 from 212 into 221
    if(n>1 && numsel[1] < numsel[2]) std::swap(numsel[1], numsel[2]);
...
std::map<int,long long> cache_pair;
